...
func LoginHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := httpx.Parse(r, &req); err != nil {
			response.Write(w, response.ErrResp(-2, usercode.Login, response.InvalidParam, err.Error()), nil)
			return
		}

		if err := svcCtx.Validate.Struct(req); err != nil {
			response.Write(w, response.ErrResp(-1, usercode.Login, response.MissingParam, err.Error()), nil)
			return
		}
